...
// subtitleHTTP bounds every OpenSubtitles call; an unbounded client can hang
// forever and leak the fetch goroutine.
var subtitleHTTP = &http.Client{Timeout: 20 * time.Second}
...
func DownloadSubtitle(fileID int) (string, error) {
	...
	srtResp, err := subtitleHTTP.Get(dlResp.Link)
	if err != nil {
		return "", err
	}
