...
func bookmarksPath() string {
	return config.Dir() + "/bookmarks.json"
}
...
func StartBookmarksTUI() {
	...
	// Alt screen to match History: inline mode paints over the main menu's
	// stale frame and leaves ragged remnants behind.
	...
	if bm, ok := finalModel.(*bookmarksModel); ok && bm.forceExit {
		ExitApp(0)
	}
