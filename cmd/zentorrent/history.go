...
func saveHistory(entries []HistoryEntry) {
	...
	os.WriteFile(config.HistoryPath(), data, 0644)
}
...
func ClearHistory() error {
	return os.Remove(config.HistoryPath())
}
...
func StartHistoryTUI() {
	// Load the FULL history, not a display-sized slice: the delete handler
	// saves this list back to disk, so truncating here would silently wipe
	// every entry older than what fits on screen.
	entries := GetHistory(0)
	...
	if hm, ok := finalModel.(historyModel); ok && hm.forceExit {
		ExitApp(0)
	}
